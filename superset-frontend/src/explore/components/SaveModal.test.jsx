...
test('shows duplicate name warning when chart name already exists', async () => {
  const wrapper = getWrapper();
  ...
  // Mock the action to throw duplicate name error
  wrapper.instance().props.actions.updateSlice = jest.fn(() =>
    Promise.reject(duplicateNameError),
  );
  wrapper.instance().props.actions.removeSaveModalAlert = jest.fn();

  // Set up state for testing
  wrapper.setState({
    newSliceName: 'Duplicate Chart Name',
    action: 'overwrite',
  });

  // Call saveOrOverwrite
  try {
    await wrapper.instance().saveOrOverwrite(false);
  } catch (e) {
    // Error is expected
  }
  ...
  // Check if the alert is set with the duplicate name message
  expect(wrapper.state().alert).toContain('A chart with the name');
  expect(wrapper.state().alert).toContain('Duplicate Chart Name');
  expect(wrapper.state().alert).toContain('already exists');
});
